    ...
    <div class="max-w-7xl mx-auto space-y-8 animate-fadeIn">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-4 space-y-4">
           <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
              <div class="px-6 py-4 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                ...
                <span class="px-2 py-0.5 bg-primary-50 text-primary-700 rounded-md text-[10px] font-bold">{{ campaigns.length }}</span>
              </div>
              <div class="p-2 max-h-[600px] overflow-y-auto custom-scrollbar">
                @if (isLoading) {
                  <div class="p-4 space-y-2">
                    @for (i of [1,2,3,4,5]; track i) {
                      <div class="h-12 bg-slate-50 animate-pulse rounded-xl"></div>
                    }
                  </div>
                } @else {
                  ...
                  <button *ngFor="let tag of campaigns" 
                          ...
                          [class.bg-primary-600]="selectedTag === tag"
                          [class.text-white]="selectedTag === tag"
                          [class.shadow-lg]="selectedTag === tag"
                          [class.shadow-primary-200]="selectedTag === tag"
        ...
        <div class="lg:col-span-8 space-y-6 relative">
           @if (isSelecting) {
             <div class="absolute inset-0 bg-slate-50/40 backdrop-blur-[2px] z-20 flex items-center justify-center rounded-[2.5rem]">
                <div class="flex flex-col items-center gap-3">
                   <div class="w-10 h-10 border-4 border-primary-500/20 border-t-primary-600 rounded-full animate-spin"></div>
                   <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Loading Analytics...</p>
                </div>
             </div>
           }
           ...
           <div *ngIf="selectedTag" class="space-y-6 animate-fadeIn">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                  ...
                  <h4 class="text-2xl font-black text-primary-600">{{ campaignTotalClicks }}</h4>
                </div>
              </div>
              ...
              <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                <div class="flex items-center justify-between mb-8">
                  <h3 class="text-lg font-bold text-slate-800 tracking-tight flex items-center gap-2">
                    <span class="text-primary-600">#</span>{{ selectedTag }} Performance
                  </h3>
